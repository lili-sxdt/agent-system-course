...
// 生成学习区三级侧栏：部分(可折叠) → 章(可折叠) → 节(锚点链接)
function genLearnSidebar() {
  const groups: any[] = []
  for (const p of Object.keys(PART_TITLES).map(Number).sort((a, b) => a - b)) {
    ...
    const chapters = fs.readdirSync(dir)
      ...
      .map((f) => {
        ...
        const shortName = shorten(extractH1(content)) // 例：'第5章 跑通第一个 Agent'
        const sections = extractH2(content)
        return {
          text: shortName,
          link: `/learn/part-${p}/${stem}`,
          collapsed: true,
          items: sections.map((s, i) => ({
            text: shorten(s),
            link: `/learn/part-${p}/${stem}#sec-${i + 1}`,
          })),
        }
      })
